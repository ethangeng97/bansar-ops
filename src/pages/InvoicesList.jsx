...
export default function InvoicesList({ user, onBack }) {
  ...
  return (
    <>
      <h1 className="page-title">开票/收票记录</h1>
      ...
      <div className="page-section-bar">
        <div style={{ display: "flex", gap: 6 }}>
          {[
            ...
            <button key={k} className={"btn" + (kindFilter === k ? " primary" : "")}
                    onClick={() => setKindFilter(k)}
                    style={{ padding: "4px 12px", fontSize: 12 }}>{label}</button>
          ))}
        </div>
        <input className="field-input" placeholder="发票号 / 对方"
        ...
        <button className="btn" onClick={() => { setFilters({keyword:"", date_from:"", date_to:""}); setTimeout(load,0); }}>重置</button>
      </div>

      <div className="page-section-bar" style={{ background: "#fff" }}>
        <span style={{ flex: 1, color: "var(--shell-text-2)", fontSize: 12 }}>
          共 <b>{summary.count}</b> 张
          {Object.entries(summary.byCcy).map(([c, v]) => ` · ${c} ${v.toFixed(2)}`).join("")}
        </span>
        <button className="btn" onClick={() => setImporting(true)}>⬆ CSV 导入</button>
        <button className="btn primary" onClick={() => setEditing({})}>+ 新建{direction === "AR" ? "开票" : "收票"}</button>
      </div>
      ...
      <div className="page-card" style={{ padding: 0, overflow: "auto" }}>
        {loading ? (
          ...
          <div className="empty-state empty-text">暂无{direction === "AR" ? "开票" : "收票"}记录</div>
        ) : (
